...
class AdmissionsWorkflow:
    ...
    def __init__(self, knowledge_base: KnowledgeBase, logger: TraceLogger | None = None):
        ...
        self.synthesis = GeminiSynthesisService()
    ...
    def orchestrate(self, query: str) -> dict[str, Any]:
        ...
        return {
            ...
            "clarity_score": min(10, max(1, len(tokens(query)) // 2 + 3)),
            "risk_level": risk,
            "need_summary": False,
            "need_clarification": not is_chitchat and not is_out_of_scope and len(tokens(query)) < 4,
            "need_human": risk == "high",
            "next_agent": "synthesis" if is_chitchat or is_out_of_scope else "admissions_analyst",
        }
    ...
    def analyze(self, query: str, orchestration: dict[str, Any], profile: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "rewritten_query": f"{query}. Trả lời dựa trên bằng chứng đã kiểm tra nguồn và nêu rõ giới hạn dữ liệu.",
            "intent": orchestration["intent"],
            "entities": profile,
            "missing_information": missing,
            "sub_tasks": [
                "Tìm bằng chứng liên quan",
                "Kiểm tra độ tin cậy và tính cập nhật",
                "Tổng hợp câu trả lời có nguồn",
            ],
        }
    ...
    def validate(self, evidence: list[Evidence], risk: str) -> dict[str, Any]:
        ...
        return {
            "passed": passed,
            "source_check_passed": bool(valid),
            "evidence_count": len(valid),
            "avg_relevance": round(avg_relevance, 3),
            "avg_source_trust": round(avg_trust, 3),
            "risk_level": risk,
            "needs_human": risk == "high" or not passed,
            "rejection_reasons": [] if passed else ["Không đủ evidence đạt ngưỡng source/relevance"],
        }
    ...
    def answer(self, query: str, *, context=None, history=None, profile=None, request_id=None) -> dict[str, Any]:
        ...
        if orchestration["is_chitchat"]:
            validation = self._empty_validation(passed=True)
            ...
            validation = self._empty_validation(passed=False, reason="Yêu cầu nằm ngoài phạm vi tư vấn tuyển sinh")
            ...
            if orchestration["need_clarification"]:
                ...
                response = self.synthesis.synthesize(
                    ...
                    human_required=validation["needs_human"],
                    profile=profile,
                    history=history or [],
                )
        for agent, payload in (
            ("orchestrator", orchestration),
            ("admissions_analyst", analysis),
            ("searcher", {"evidence": [asdict(item) for item in evidence]}),
            ("validator", validation),
            ("synthesis", {"response": response, "citation_count": len(usable), "model": self.synthesis.model_name if self.synthesis.configured else "fallback"}),
        ):
        ...
        result = {
            ...
            "analysis": analysis,
            "validation": validation,
            "evidence": [asdict(item) for item in usable],
            "sources": [asdict(self.kb.sources[item.source_id]) for item in usable],
            "prompt_xml": XmlPrompt.build(request_id, query, context, history, profile, usable),
        }
    ...
    @staticmethod
    def _empty_validation(passed: bool, reason: str = "") -> dict[str, Any]:
        return {
            "passed": passed,
